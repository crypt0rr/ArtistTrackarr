...
footer. The current release is `v0.22.0`, which is also displayed by local
...
- Pushing a tag such as `v0.22.0` publishes `0.22.0`, `0.22`, and `latest`.
...
```console
ARTIST_TRACKARR_IMAGE=ghcr.io/crypt0rr/artist-trackarr:0.22.0 docker compose up -d
```
