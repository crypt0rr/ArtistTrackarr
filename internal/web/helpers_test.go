...
func TestTemplatesRenderRepresentativePageData(t *testing.T) {
	...
	data := PageData{
		Title: "Template smoke", Version: "0.46.1", User: &UserView{ID: user.ID, Email: user.Email, Username: user.Username, Role: user.Role, Timezone: user.Timezone, ReminderTime: user.ReminderTime}, CSRF: "csrf", SetupNeeded: true, Query: "template",
